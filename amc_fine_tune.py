...
def parse_args():
    ...
    parser.add_argument('--model', default='mobilenet_0.5flops', type=str, help='name of the model to train')
    ...
    parser.add_argument('--n_epoch', default=30, type=int, help='number of epochs to train')
    ...
    parser.add_argument('--data_root', default='dataset/imagenet', type=str, help='dataset path')
    # resume
    parser.add_argument('--ckpt_path', default='./checkpoints/mobilenet_0.5flops_export.pth', type=str, help='checkpoint path to resume from')
    # run eval
...
def get_model():
    ...
    if args.model == 'mobilenet':
        ...
        net = MobileNet(n_class=20)
    elif args.model == 'mobilenet_0.5flops':
        from models.mobilenet import MobileNet
        net = MobileNet(n_class=20, profile='0.5flops')
    else:
...
def train(epoch, train_loader):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        progress_bar(batch_idx, len(train_loader), 'Loss: {:.3f} | Acc1: {:.3f}% | Acc3: {:.3f}%'
                     .format(losses.avg, top1.avg, top5.avg))
    writer.add_scalar('loss/train', losses.avg, epoch)
    writer.add_scalar('acc/train_top1', top1.avg, epoch)
    writer.add_scalar('acc/train_top3', top5.avg, epoch)
...
def test(epoch, test_loader, save=True):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            progress_bar(batch_idx, len(test_loader), 'Loss: {:.3f} | Acc1: {:.3f}% | Acc3: {:.3f}%'
                         .format(losses.avg, top1.avg, top5.avg))

    if save:
        writer.add_scalar('loss/test', losses.avg, epoch)
        writer.add_scalar('acc/test_top1', top1.avg, epoch)
        writer.add_scalar('acc/test_top3', top5.avg, epoch)
...
if __name__ == '__main__':
    ...
    train_loader, val_loader, n_class = get_split_dataset(args.dataset, args.batch_size, args.n_worker,3000,
